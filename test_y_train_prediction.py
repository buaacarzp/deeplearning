...
def model(X_train, Y_train, X_test, Y_test, learning_rate=0.009, 
    ...
    #对自己的图像进行检测
    pic=cv.imread("./images/img_1.png")
    print("shape is:",pic.shape,type(pic))
    #picture_result=sess.run(predict_op,feed_dict={X:X_test, Y:Y_test})#至于这里为什么传入predict_op就能预测要搞懂
    cv.imshow("picture",pic)
    res_pic=cv.resize(pic,(64,64))
    IMG_H,_,_=res_pic.shape
    image = np.reshape(res_pic, [1,IMG_H,IMG_H,3])
    #开始运行
    with tf.Session() as sess:
        #初始化参数
        sess.run(init)
        saver=tf.train.Saver()
        saver = tf.train.import_meta_graph('./Model/new_model.ckpt-10.meta')
        saver.restore(sess,tf.train.latest_checkpoint('./Model'))
        predict_op=tf.arg_max(Z3,1)
        picture_result=sess.run(predict_op,feed_dict={X:image})
        print("your picture result is:\n",picture_result)
        #print("the picture's real result is:\n",Y_test_orig)
        '''
        #开始遍历数据集
        for epoch in range(num_epochs):
            ...
            save_path=saver.save(sess,'./Model/new_model.ckpt',global_step=10)
            #save_path=saver.save(sess,'./model2'+str(epoch)+str('.ckpt'),global_step=5)
        ...
        '''
        return (Z3,costs,picture_result)
